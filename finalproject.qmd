...
server<-function(input, output, session){
  ...
  textModel <- reactive ({
    if(input$team == "All" | input$outdoors == "Both" | input$rival == "Both" | input$playoff == "Both" | 
      ...
      tmodel <- tmodel |> add_predictions(logmodel()) |> 
        mutate(exppred = exp(pred)) |> mutate(upsetprob = exppred / (1+exppred)) |>
        rename("Probability of Being Involved in an Upset" = upsetprob)
      tmodel |> select(ncol(tmodel))
    }
  ...
  output$prob <- renderTable({textModel()})  
